defmodule Onelist.Livelog.Redaction do
  ...
  @secret_patterns [
    ...
    {~r/(api[_-]?key|token|secret)\s*[:=]\s*['"]?[a-zA-Z0-9_\-]{16,}['"]?/i, "[REDACTED:credential]"},
  ...
  @infra_patterns [
    ...
    {~r/\b(?!127\.\d+\.\d+\.\d+)(?!0\.0\.0\.0)(?!192\.0\.2\.)(?!198\.51\.100\.)(?!203\.0\.113\.)((?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\b/, "[REDACTED:ip]"},
  ...
  def redact(text) when is_binary(text) do
    case check_hard_blockers(text) do
      ...
      :ok ->
        redacted =
          text
          |> apply_identity_protection()  # Layer 2 - CANNOT SKIP
          |> apply_secret_patterns()       # Layer 3
          |> apply_infra_patterns()        # Layer 4
          |> apply_pii_patterns()          # Layer 5
